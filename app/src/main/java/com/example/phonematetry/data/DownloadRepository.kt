...
import com.example.phonematetry.worker.DownloadWorker
...
data class DownloadWorkInfo(val modelName: String, val workId: String)
...
interface DownloadRepository {
    ...
    fun getEnqueuedOrRunningWorkInfos(): List<DownloadWorkInfo>
}
...
class DefaultDownloadRepository(
            ...
            if (workInfo != null) {
                when (workInfo.state) {
                    ...
                    androidx.work.WorkInfo.State.SUCCEEDED -> {
                        ...
                        sendNotification(
                            title = "下载完成",
                            text = "模型 ${model.name} 下载完成",
                            modelName = model.name,
                        )

                        val startTime = downloadStartTimeSharedPreferences.getLong(model.name, 0L)
                        val duration = System.currentTimeMillis() - startTime
                        Log.d(TAG, "Model ${model.name} download completed in ${duration}ms")
                    ...
                    androidx.work.WorkInfo.State.CANCELLED -> {
                        ...
                        val startTime = downloadStartTimeSharedPreferences.getLong(model.name, 0L)
                        val duration = System.currentTimeMillis() - startTime
                        Log.d(TAG, "Model ${model.name} download failed after ${duration}ms: $errorMessage")
                        downloadStartTimeSharedPreferences.edit { remove(model.name) }
                    }
    ...
    /**
     * Retrieves a list of WorkInfo objects representing WorkManager work items that are either
     * enqueued or currently running.
     */
    override fun getEnqueuedOrRunningWorkInfos(): List<DownloadWorkInfo> {
        val workQuery =
            WorkQuery.Builder.fromStates(listOf(androidx.work.WorkInfo.State.ENQUEUED, androidx.work.WorkInfo.State.RUNNING)).build()
